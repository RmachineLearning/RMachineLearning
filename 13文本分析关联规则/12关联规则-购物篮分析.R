...
#将tr_list转化成为transactions，即apriori函数可以处理的对象 
...
#
data  = read.csv()
#一行代表一个购物篮，列代表篮子的数量，把所有的商品放在一列用数字标签标明
#该物品是属于哪一类
bsk = gl(nrow(data), ncol(data))
#data.frame不能直接转变成向量，必须先转化为矩阵，再转化为向量
item = as.vector(t(as.matrix(data)))
data_frame = data.frame(TID = bsk, item = item)
trans.list = split(data_frame[,'item'], data_frame[, 'TID'])
#转化成apriori函数处理的对象
trans3 = as(trans.list, "transactions")
...
#{bread} => {butter} 前项 =》 后项 为2项；{bread，jielly} => {butter} 前    项 =》 后项 为3项
...
inspect(rules[1:3])
#过滤掉lift小于等于1的关联规则，其中rules@quality包含support confidence lifts三个指标 
